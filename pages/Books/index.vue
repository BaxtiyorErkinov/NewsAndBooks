					...
					<v-row class="justify-center">
						<v-col cols="12" md="5" v-for="result in result" :key="result.id">
							<v-card flat class="grey lighten-3 pa-5">
								<v-img height="200" :src="`https://actions.uz${result.cover}`"></v-img>
								<div class="text-center pa-3 mt-5">
									<h2 class="mb-2">{{result.title}}</h2>
								</div>
								<div class="text-right">
									<a :href="`https://actions.uz${result.book}`" class="text-decoration-none"><v-btn class="white--text" color="teal accent-3">DOWNLOAD</v-btn></a>
								</div>
							</v-card>
						</v-col>
					</v-row>
...
<script>
	export default{
		...
		methods:{
			async get_result(){
				await this.$axios.post('https://actions.uz/api/booksearch/', this.form)
				.then(res => {
